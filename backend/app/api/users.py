...
async def get_users(
    ...
    try:
        ...
        if search:
            ...
            base_query += """ AND (
                u.USER_ID LIKE %(search)s OR
                u.FIRST_NAME LIKE %(search)s OR
                u.LAST_NAME LIKE %(search)s OR
                u.EMAIL_ADDRESS LIKE %(search)s OR
                u.EMPLOYEE_ID LIKE %(search)s
            )"""
            params["search"] = search_term
        
        # Add user type filter
        if user_type:
            base_query += " AND u.USER_TYPE = %(user_type)s"
            params["user_type"] = user_type
        
        # Add status filter
        if status:
            base_query += " AND u.STATUS = %(status)s"
        ...
        if profile_id:
            base_query += """
                AND EXISTS (
                    ...
                    AND up.PROFILE_ID = %(profile_id)s
                )
            """
        ...
        base_query += " OFFSET %(skip)s ROWS FETCH NEXT %(limit)s ROWS ONLY"
        params["skip"] = skip
        params["limit"] = limit
...
async def create_user(
    ...
    try:
        ...
        cursor.execute("SELECT USER_KEY FROM USERS WHERE USER_ID = %s", (user_data.USER_ID,))
        if cursor.fetchone():
            raise HTTPException(
                status_code=409,
        ...
        placeholders = ["%s" if i < len(data) else f"{v}" for i, v in enumerate(values)]
...
async def update_user_status(
    ...
    try:
        ...
        cursor.execute("""
            UPDATE USERS
            SET STATUS = %s, UPDATED_DATE = GETDATE()
            WHERE USER_KEY = %s
